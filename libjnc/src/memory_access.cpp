...
Java_jnc_provider_NativeMethods_put##fname(                 \
    ...
    checkNullPointer(env, addr, /*void*/);                  \
    if (likely(checkNullAndRange(env, array, off, len)))    \
        env->Get##jni##ArrayRegion(array, off, len, addr);  \
}
...
Java_jnc_provider_NativeMethods_get##name(                  \
        JNIEnv *env, jobject, jlong laddr, jlong ltype) {   \
    auto paddr = j2vp(laddr);                               \
    auto ptype = j2c(ltype, ffi_type);                      \
...
EXTERNC JNIEXPORT void JNICALL                              \
Java_jnc_provider_NativeMethods_put##name(                  \
        JNIEnv *env, jobject, jlong address, jtype value) { \
    auto paddr = j2c(address, native);                      \
    checkNullPointer(env, paddr, /*void*/);                 \
    *paddr = j2n(value);                                    \
}                                                           \
EXTERNC JNIEXPORT jtype JNICALL                             \
